...
export default function RegisterScreen({ onGoLogin, onGoBack }) {
  ...
  function validateFullNameField() {
    ...
    if (!value) {
      setFieldError('fullName', 'Vui lòng nh?p h? và tên.');
      return false;
    }
    if (value.length < 2 || value.length > 50) {
      setFieldError('fullName', 'H? tên ph?i t? 2 d?n 50 ký t?.');
      return false;
    }
  ...
  async function validateUserNameField() {
    ...
    if (!value) {
      setFieldError('userName', 'Vui lòng nh?p username.');
      return false;
    }
    if (value.length < 3 || value.length > 20) {
      setFieldError('userName', 'Username ph?i t? 3 d?n 20 ký t?.');
      return false;
    }
    if (!/^[a-zA-Z0-9_]+$/.test(value)) {
      setFieldError('userName', 'Username ch? du?c dùng ch?, s? và d?u g?ch du?i.');
      return false;
    }
    ...
    try {
      ...
      if (userNameTaken) {
        setFieldError('userName', 'Username này dã t?n t?i.');
        return false;
      }
    } catch {
      // Không ch?n khi API check l?i; dang ký s? ki?m tra l?i phía server.
    }
  ...
  async function validateEmailField() {
    ...
    if (!value) {
      setFieldError('email', 'Vui lòng nh?p email.');
      return false;
    }
    if (value.length < 6 || value.length > 100) {
      setFieldError('email', 'Email ph?i t? 6 d?n 100 ký t?.');
      return false;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
      setFieldError('email', 'Email không h?p l?.');
      return false;
    }
    ...
    try {
      ...
      if (emailTaken) {
        setFieldError('email', 'Email này dã du?c s? d?ng.');
        return false;
      }
    } catch {
      // Không ch?n khi API check l?i; dang ký s? ki?m tra l?i phía server.
  ...
  function validatePasswordField() {
    if (!password) {
      setFieldError('password', 'Vui lòng nh?p m?t kh?u.');
      return false;
    }
    if (password.length < 6 || password.length > 32) {
      setFieldError('password', 'M?t kh?u ph?i t? 6 d?n 32 ký t?.');
      return false;
    }
  ...
  function validateConfirmPasswordField() {
    if (!confirmPassword) {
      setFieldError('confirmPassword', 'Vui lòng nh?p l?i m?t kh?u.');
      return false;
    }
    if (confirmPassword !== password) {
      setFieldError('confirmPassword', 'M?t kh?u xác nh?n chua kh?p.');
      return false;
    }
    ...
    >
      ...
      >
        <View style={styles.headerRow}>
          ...
          />
          <Text style={styles.headerTitle}>Ðang ký tài kho?n m?i</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          >
            ...
            <Text style={styles.termsText}>
              Tôi d?ng ý v?i các <Text style={styles.termsLink}>Ði?u kho?n d?ch v?</Text> và{' '}
              <Text style={styles.termsLink}>Chính sách b?o m?t</Text> c?a FastMark.
            </Text>
          </Pressable>
          ...
          >
            <Text style={styles.primaryButtonText}>
              {isLoading ? 'Ðang dang ký...' : 'Ðang ký'}
            </Text>
          </Pressable>

          <AuthDivider label="Ho?c dang ký b?ng" />
