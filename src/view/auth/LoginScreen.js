    ...
    try {
      ...
      const payload =
        loginError && typeof loginError === 'object'
          ? loginError
          : { message: String(loginError || 'Ðang nh?p th?t b?i.'), field: '' };

      const message = payload.message || 'Ðang nh?p th?t b?i.';
      ...
      } else if (/không t?n t?i|không tìm th?y/i.test(message)) {
        setFieldErrors({ login: message, password: '' });
      ...
      >
        <View style={styles.brandWrap}>
          <Image
            ...
            resizeMode="cover"
          />
          <Text style={styles.brandTitle}>Ðang nh?p tài kho?n</Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          >
            <Text style={styles.primaryButtonText}>
              {isLoading ? 'Ðang dang nh?p...' : 'Ðang nh?p'}
            </Text>
          </Pressable>

          <Pressable onPress={() => onGoForgot?.()} style={styles.forgotLinkWrap} hitSlop={8}>
            <Text style={styles.forgotLink}>Quên m?t kh?u?</Text>
          </Pressable>
        ...
        >
          <Text style={styles.registerButtonText}>Ðang ký tài kho?n m?i</Text>
        </Pressable>
      </ScrollView>
    </KeyboardAvoidingView>
  );
